...
mrprot.lBaseResolution=EPI{2}.hdr().Meas.BaseResolution;
mrprot.lPhaseEncodingLines=EPI{2}.hdr().Meas.lPhaseEncodingLines;
mrprot.dInPlaneRot=EPI{2}.hdr().MeasYaps.sSliceArray.asSlice{1}.dInPlaneRot;
mrprot.lSlices=EPI{2}.hdr().Meas.NSlcMeas;
mrprot.lPhaseFoV=EPI{2}.hdr().MeasYaps.sSliceArray.asSlice{1}.dPhaseFOV;

EPI=squeeze(EPI{2}.image());
EPIacc=mapVBVD(filename2);
EPIacc=squeeze(EPIacc{2}.image());
